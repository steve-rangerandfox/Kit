...
/**
 * Prominent banner prepended to every managed Project Control Canvas. It makes
 * the one-way contract visible in the artifact itself (invariant #14): the
 * Master Project List is authoritative, this Canvas is a generated projection,
 * and edits made here are not read back and will be overwritten on the next
 * sync. This is a communication safeguard layered on top of the deterministic
 * full re-render (which actually discards any manual edit) — never the only
 * safeguard.
 */
export const GENERATED_VIEW_NOTICE =
  '> 🔒 **Generated view — do not edit here.**\n' +
  "> This canvas is generated by Kit from the **Master Project List** (the authoritative source of truth). " +
  'Edit the project’s row in the Master Project List; changes made directly to this canvas are **not** read back ' +
  'and will be **overwritten** on the next sync.'

const NOTICE_PREFIX = `${GENERATED_VIEW_NOTICE}\n\n`

/** Remove a leading generated-view notice if present, so re-rendering an
 *  already-rendered body never stacks the banner (keeps the render idempotent). */
function stripGeneratedViewNotice(markdown: string): string {
  return markdown.startsWith(NOTICE_PREFIX) ? markdown.slice(NOTICE_PREFIX.length) : markdown
}
...
export function renderProjectControlCanvas(templateMarkdown: string, row: NormalizedRow): string {
  // Source-of-truth guarantee: the rendered body derives ONLY from the stored
  // template snapshot + the authoritative Sheet row. Any prior canvas content
  // (including accidental manual edits) is never an input, so it can never
  // become source data. A previously rendered notice is stripped first so the
  // banner is applied exactly once.
  const filled = stripGeneratedViewNotice(templateMarkdown)
    .split('\n')
    .map((line) => {
  ...
  const spine = ['Project Number', 'Client', 'Project Name']
    ...
    .filter(Boolean)
    .join('_')
  const body = spine ? filled.replace(/2x{2,}\s+client\s+project/i, spine) : filled
  return `${NOTICE_PREFIX}${body}`
}
