...
| Pilots (Visual Dev) | Bounded evidence-driven pilot attached to one project: references, append-only evidence, generations + human acceptance, material maps, C4D/Redshift validation, deterministic completeness-gated human recommendation; read-only Canvas projection | `/kit pilot` → `bolt/src/handlers/pilots.ts` → `src/lib/pilots/service.ts` | `src/lib/pilots/` | Railway (Bolt); **Supabase authoritative** | `npx tsx --test src/lib/pilots/*.test.ts` | `pilots/types.ts`, then `service.ts`, `completeness.ts`, `metrics.ts` | V |
