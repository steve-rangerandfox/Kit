...
- **Frame.io v4 auth** uses Adobe IMS refresh-token flow (`src/lib/frameio/auth.ts`) — env vars are `ADOBE_CLIENT_ID`, `ADOBE_CLIENT_SECRET`, `ADOBE_REFRESH_TOKEN`, `FRAMEIO_ACCOUNT_ID`, `FRAMEIO_WORKSPACE_ID`. There are three Frame.io provisioning code paths (Inngest agent, `src/lib/provisioner/` service, and `src/lib/mcp/provisioners.ts`); all three are now on v4 — keep them in sync if you change one.
